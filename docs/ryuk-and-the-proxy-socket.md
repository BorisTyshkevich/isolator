...
### Workarounds on macOS

Three viable paths, in increasing order of work:

1. **Disable Ryuk and rely on per-test-suite cleanup.** Set
   `TESTCONTAINERS_REUSE_ENABLE=false` and have the test suite end with an
   explicit `docker rm -f $(docker ps -aq -f label=org.testcontainers.session-id=...)`
   pass. The proxy's container-list filtering means each user only sees
   their own containers, so a blanket label-cleanup is safe. **This is the
   pragmatic short-term answer.**

2. **Run a cron-based reaper on macOS.** A background process running as
   root sweeps every N minutes for containers whose owner-label user has
   no active proxy connections. Bounded leak window without the
   in-container socket dependency. Modest implementation effort.

3. **Add a TCP listener to the proxy.** Have docker-proxy listen on
   `127.0.0.1:NNNN` on macOS in addition to its Unix socket, with a
   shared-secret auth scheme (since `SO_PEERCRED` doesn't work over TCP).
   testcontainers can be configured via
   `TESTCONTAINERS_HOST_OVERRIDE=host.docker.internal` and
   `TESTCONTAINERS_RYUK_DOCKER_SOCKET_OVERRIDE=tcp://host.docker.internal:NNNN`
   to point Ryuk at the TCP endpoint. The container reaches the host via
   OrbStack's normal VM-to-host networking, no socket-bridge needed. Most
   work; cleanest end-state.

We currently ship option 1 as the operational guidance. Option 3 is on the
roadmap for any deployment where Ryuk is required.
...
| Platform                         | Bind allowed | Connection works |
|----------------------------------|--------------|------------------|
| Linux (host Docker, rootless)    | yes          | yes              |
| macOS (OrbStack)                 | yes          | no (VM boundary) |
| macOS (Docker Desktop)           | yes          | no (VM boundary) |

The fix is correct on every platform; macOS just needs an additional step
(option 1, 2, or 3 above) to actually deliver Ryuk's functionality.
